...
import type { CambiarEstadoPlanSolicitud } from '@/types/curriculo';
...
export async function PATCH(req: NextRequest, { params }: Params): Promise<NextResponse> {
  ...
  let body: CambiarEstadoPlanSolicitud;
  try {
    body = await req.json() as CambiarEstadoPlanSolicitud;
  } catch {
  ...
  const resultado = await llamarAutenticado<void>(
    sesion,
    `/api/v1/curriculo/planes/${id}/estado`,
    { method: 'PATCH', body },
  );
