...
import type { CambiarEstadoDetalleSolicitud } from '@/types/curriculo';
...
export async function PATCH(req: NextRequest, { params }: Params): Promise<NextResponse> {
  ...
  let body: CambiarEstadoDetalleSolicitud;
  try {
    body = await req.json() as CambiarEstadoDetalleSolicitud;
  } catch {
  ...
  const resultado = await llamarAutenticado<void>(
    sesion,
    `/api/v1/curriculo/planes/${id}/detalles/${idDetalle}/estado`,
    { method: 'PATCH', body },
  );
