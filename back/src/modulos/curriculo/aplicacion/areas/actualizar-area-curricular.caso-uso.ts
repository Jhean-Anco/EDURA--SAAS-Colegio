...
export class ActualizarAreaCurricularCasoUso {
  ...
  async ejecutar(
    ...
    const area = await this.repositorio.obtenerAreaBase(entrada.id, alcance.institucionId);
    ...
    const nombreNorm = entrada.nombre ? entrada.nombre.trim().toUpperCase() : undefined;

    if (codigoNorm) {
      if (await this.repositorio.existeCodigoAreaEnInstitucion(codigoNorm, alcance.institucionId, entrada.id)) {
        throw new AreaCodigoDuplicadoError();
      }
    }
    if (nombreNorm) {
      if (await this.repositorio.existeNombreAreaEnInstitucion(nombreNorm, alcance.institucionId, entrada.id)) {
        throw new AreaNombreDuplicadoError();
      }
    }
    if (entrada.orden !== undefined) {
      if (await this.repositorio.existeOrdenAreaEnInstitucion(entrada.orden, alcance.institucionId, entrada.id)) {
