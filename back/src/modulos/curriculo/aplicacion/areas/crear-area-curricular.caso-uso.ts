...
export class CrearAreaCurricularCasoUso {
  ...
  async ejecutar(
    ...
    if (await this.repositorio.existeCodigoAreaEnInstitucion(codigoNorm, alcance.institucionId)) {
      throw new AreaCodigoDuplicadoError();
    }
    if (await this.repositorio.existeNombreAreaEnInstitucion(nombreNorm, alcance.institucionId)) {
      throw new AreaNombreDuplicadoError();
    }
    if (await this.repositorio.existeOrdenAreaEnInstitucion(entrada.orden, alcance.institucionId)) {
